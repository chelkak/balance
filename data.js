...
window.DATA_JS_VERSION="100 · 21.08.2026 12:38";
...
const SPHERES=[
 ...
 {code:"family",name:"Семья",ic:"🏠",min:"полчаса вместе без телефонов",
  ...
  spark:["Позвонить родителям","Поужинать вместе без экранов","Сделать что-то по дому без просьбы","Сказать вслух, за что благодарен","Договориться о свидании на неделе","Убрать телефон на весь вечер"]},
 {code:"friends",name:"Друзья",ic:"🤝",min:"написать другу",
  ...
  spark:["Написать тому, о ком вспомнил","Позвать кого-нибудь на выходных","Ответить на висящее сообщение","Помочь одному человеку","Поблагодарить кого-то вслух","Сходить туда, где живые люди"]},
 {code:"career",name:"Карьера",ic:"💼",min:"25 минут над главной задачей",
  ...
  spark:["25 минут над самой важной задачей","Написать письмо, которое откладывал","Составить список на завтра","Записать вчерашние траты","Посмотреть, сколько ушло за неделю","Отложить любую сумму"]},
 {code:"self",name:"Саморазвитие",ic:"🌱",min:"10 минут учёбы",
  ...
  spark:["10 минут по этому проекту","Прочитать пять страниц","Выписать, что тревожит, списком","Пять минут сидеть без телефона","Дойти до воды или деревьев","Записать три вещи, за которые благодарен"]},
    ...
    if(пораСверять()){
      ...
      if(итоги.length) шапка.innerHTML=
        ...
        итоги.map(о=>`<div style="margin-top:7px">${о.n}: <b>${о.было} → ${о.стало}</b> `+
          `<span class="dim">${о.стало>о.было?"выросло":"просело"}</span></div>`).join("");
    }
  }
    ...
    `<div style="display:flex;justify-content:space-between;align-items:center">
     ...
     ${(v && !пораСверять())
        ? `<div class="dim" style="margin-top:16px;padding-top:13px;border-top:1px solid var(--brd)">
            ...
            Оценка держится неделю. Следующая сверка через ${
              Math.max(0,СВЕРКА_ЧЕРЕЗ-днейСоСверки())} ${
              plural(Math.max(0,СВЕРКА_ЧЕРЕЗ-днейСоСверки()),"день","дня","дней")}.</div>`
